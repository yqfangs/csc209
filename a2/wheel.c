...
char **prune_word_list(char **words, int len, int *words_remaining) {
    ...
    char **word_list = malloc(sizeof(char*) * (num_word + 1));
    ...
    word_list[num_word] = NULL;
    
    *words_remaining = num_word;
    return word_list;
}
...
char **get_word_list_of_length(char **words, int *len) {
    ...
    length = strtol(input, &leftover, 10);
    char **word_list_len = prune_word_list(words, length, &words_remaining);
	...
	word_list_len = prune_word_list(words, length, &words_remaining);
    }
    *len = length;
